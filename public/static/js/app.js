        ...
        <el-submenu :index="group.group_name">
            ...
            <template v-for="item in group.items ">
                <el-menu-item :index="baseURL+item.url"><i :class="item.icon"></i>
                    <el-link :href="baseURL+item.url" target="_blank">{{item.item_name}}</el-link>
                </el-menu-item>
            </template>
        </el-submenu>
     </template>
...
Vue.component('msg-notify', {
    template: `
        <el-dropdown size="small" class="mr-6">
            <span class="el-dropdown-link">
                 <el-badge :value="count" :max="99" class="item cursor-text">
                     <i class="text-teal-500 el-icon-message-solid"></i>
                 </el-badge>
            </span>
            <el-dropdown-menu slot="dropdown" trigger="click" v-show="_.isEmpty(notices)">
                <el-dropdown-item v-for="(value, key) in notices" :key="key">
                    <el-link :href="jumpPage(key)" target="_blank">通知消息{{ key }}</el-link>
                </el-dropdown-item>
            </el-dropdown-menu>
        </el-dropdown>
    `,
    created() {
        let handicap_update = localStorage.getItem('notice.handicap.update');
        if (!_.isEmpty(handicap_update)) {
            this.notices.handicap_update = 1;
        }
        let reject_ticket = localStorage.getItem('notice.reject_ticket');
        if (!_.isEmpty(reject_ticket)) {
            this.notices.reject_ticket = 1;
        }
        let trigger_limit = localStorage.getItem('notice.trigger_limit');
        if (!_.isEmpty(trigger_limit)) {
            this.notices.trigger_limit = 1;
        }
        let match_update = localStorage.getItem('notice.match.update');
        if (!_.isEmpty(match_update)) {
            this.notices.match_update = 1;
        }
        this.count = _.isObject(this.notices) ? Object.keys(this.notices).length : 0;
    ...
    methods: {
        handleNotices(data) {
            if (!_.isEmpty(data) && _.isString(data) && data.indexOf('{') !== -1) {
                let newNotices = JSON.parse(data);
                if (!_.isEmpty(newNotices.tid)) {
                    if (newNotices.tid === 'notice.handicap.update') {
                        this.notices.handicap_update = 1;
                    }

                    if (newNotices.tid === 'notice.reject_ticket') {
                        this.notices.reject_ticket = 1;
                    }

                    if (newNotices.tid === 'notice.trigger_limit') {
                        this.notices.trigger_limit = 1;
                    }

                    if (newNotices.tid === 'notice.match.update') {
                        this.notices.match_update = 1;
                    }
                    this.count = _.isObject(this.notices) ? Object.keys(this.notices).length : 0;
                }
        ...
        jumpPage(tid) {
            if (tid === 'handicap_update') {
                localStorage.removeItem('notice.handicap.update');
                return `#`;
            }
            if (tid === 'reject_ticket') {
                localStorage.removeItem('notice.reject_ticket');
                return `/rejected_ticket`;
            }
            if (tid === 'trigger_limit') {
                localStorage.removeItem('notice.trigger_limit');
                return `/large_ticket`;
            }
            if (tid === 'match_update') {
                localStorage.removeItem('notice.match.update');
                return `#`;
            }
        }
    },
...
Vue.component('alert-notify', {
    template: `
        <span class="alert-icon ml-48 absolute" :class="is_show ? 'block' : 'hidden'" @click="pauseAudio">
            <a href="/ticket_monitor">  
                <el-image src="/assets/img/alert.jpg"></el-image>
            </a>
            <audio id="alert-audio" src="" type="audio/mpeg">你的浏览器不支持audio标签</audio>
        </span>
    `,
    created() {
    ...
    methods: {
        async getWebsocketData() {
            let { data } = await this.$axios.post('/pusher/url')
            let pusher_url = data.message
            let ws = new WebSocket(pusher_url);
            ws.onmessage = (res) => {
                let data = _.isString(res.data) ? JSON.parse(res.data) : res.data;
                console.log(data);
                if (data.code === 0 && _.isEmpty(data.message)) {
                    console.info(data);
                } else {
                    if (['notice.reject_ticket', 'notice.trigger_limit', 'notice.large_ticket'].includes(data.tid)) {
                        this.is_show = false;
                        this.playAudio(data.tid);
                    }
                }
            };
            ws.onclose = () => {
        ...
        changeAudio(type) {
            if (type === 'notice.trigger_limit') {
                var obj = document.getElementById('alert-audio');
                obj.setAttribute('src', '/audio/alert.mp3');
                obj.setAttribute('loop', 'loop');
            }

            if (type === 'notice.large_ticket') {
                var obj = document.getElementById('alert-audio');
                obj.setAttribute('src', '/audio/large_ticket.mp3');
                obj.removeAttribute('loop');
            }

            if (type === 'notice.reject_ticket') {
                var obj = document.getElementById('alert-audio');
                obj.setAttribute('src', '/audio/reject_ticket.mp3');
                obj.removeAttribute('loop');
            }

        },
        async playAudio(type) {
            await this.changeAudio(type);
            let promise = document.getElementById('alert-audio').play();
            if (promise !== undefined) {
                promise.then(_ => {
                    console.log('Autoplay started alert audio!');
                }).catch(error => {
                    console.log(error);
                    console.log('Autoplay was prevented.');
                    console.log('Show a "Play" button so that user can start playback.');
                });
            }
        },
        pauseAudio() {
            let promise = document.getElementById('alert-audio').pause();
            if (promise !== undefined) {
                promise.then(_ => {
                    console.log('Pause play!');
                }).catch(error => {
                    console.log(error);
                    console.log('Pause was played.');
                });
            }
        }
    },
...
Vue.component('timer', {
    template: `
      <div class="flex justify-between">
        <el-switch
                @change="handleClock"
                v-model="isTimerStart"
                active-color="#13ce66"
                inactive-color="#ff4949"
                active-text="开始"
                inactive-text="暂停">
        </el-switch>
        <el-button size="mini">{{ timerText  }}</el-button>
      </div>
    `,
    computed: {
        betweenTime() {
            let diffTime = this.endTime - this.startTime;
            let s = this.toDub(parseInt(diffTime / 1000));
            let m = this.toDub(parseInt(diffTime / 60000));
            return `${m}:${s}`
        }
    },
    methods: {
        handleClock() {
            if (this.isTimerStart) {
                ...
                let endTime = _.isEmpty(this.endTime) ? 0 : this.endTime;
                this.timer = window.setInterval(() => {
                    let diffTime = Date.now() - this.startTime + endTime;
                    let s = parseInt(diffTime / 1000);
                    let m = parseInt(diffTime / 60000);
                    this.timerText = this.toDub(m) + "分" + ":" + this.toDub(s) + "秒";
                }, 1000);
            } else {
                this.endTime = Date.now();
                clearInterval(this.timer);
                this.$emit('between-time', this.betweenTime);
            }
        },
        toDub(n) {
            return n < 10 ? "0" + n : "" + n;
